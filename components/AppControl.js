...
//API METHODS
  ...
  goForAxios = () => {
    ...
    const type = "movie";
    //this.state.searchParametersType[Math.floor(Math.random() * this.state.searchParametersType.length)];
    ...
    if (pastSearches.length === 0) {
      console.log("INSIDE" + newSearch);
    } else if (pastSearches.includes(newSearch)) {
      while(newNum === page) {
...
//CHANGE PAGES
  ...
  handleChangeSearchParam = (parameter, ifAdd) => {
    ...
    if(parameter === "movie" || parameter === "series") {
      let { searchParametersType } = this.state;
      if (ifAdd) {
        searchParametersType.push(parameter);
      } else {
        searchParametersType = searchParametersType.filter(val => val != parameter);
      }
      this.setState({
        searchParametersType: searchParametersType,
      })
    } else if (services.includes(parameter)) {
  ...
  render() {
    ...
    if (this.state.currentPage === 'PROFILE') {
      currentView = <Profile changeParam={this.handleChangeSearchParam} paramLists={[this.state.searchParametersType, this.state.searchParametersService, this.state.searchParametersGenre]}/>;
    ...
    if (this.state.currentIndex === this.state.movieData.length){
      currentView = (
        <View style={styles.wait}>
          <Text style={styles.waitText}>Getting Your Movies!</Text>
          <ActivityIndicator animating={true} color={"#FF7D0D"} />
        </View>
      )
    }
    ...
    if (this.state.currentPage === 'LOAD') {
      ...
      return (
        <SafeAreaView style={styles.container}>
            {currentView}
            <NavBar toList={this.handleChangeToList} toProfile={this.handleChangeToProfile} changeBack={this.handleChangeBack}/>
        </SafeAreaView>
      )
    }
  }
